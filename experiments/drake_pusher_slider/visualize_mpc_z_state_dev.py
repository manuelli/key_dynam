...
from key_dynam.planner.planners import RandomShootingPlanner, PlannerMPPI, GradientDescentPlanner
...
from key_dynam.dynamics.models_dy import rollout_action_sequences, rollout_model

# constants
set_seed(0)

USE_FIXED_MPC_HORIZON = False
USE_SHORT_HORIZON_MPC = True
...
def main():
    ...
    planner_config['mpc']['gradient_descent']['terminal_cost_only'] = TERMINAL_COST_ONLY
    ...
    elif PLANNER_TYPE == "gradient_descent":
        planner = GradientDescentPlanner(planner_config)
    else:
    ...
    while True:
        ...
        if counter == 0 or REPLAN:
            ...
            if SEED_WITH_NOMINAL_ACTIONS:
                ...
                if mpc_out is not None:
                    action_seq_rollout_init = mpc_out['action_seq'][1:].cpu().detach()
            ...
            print("action_seq_rollout_init", action_seq_rollout_init)
            mpc_start_time = time.time()
            mpc_out = planner.trajectory_optimization(state_cur=z_cur,
                                                      ...
                                                      verbose=True,
                                                      add_constant_action_samples=True,
                                                      )
        ...
        L = len(obs_mpc_gt)
        print("state_pred_mpc.shape", state_pred_mpc.shape)
        print("L", L)
        if L == 0:
            break
        for i in range(L):
...
def test():
    ...
    B = 1
    states = torch.zeros([B, n_his, obs_dim]).cuda()
    actions = torch.zeros([B, n_his, action_dim]).cuda()

    actions.requires_grad = True
    print("actions.is_leaf", actions.is_leaf)
    dyna_net_input = {'observation': states,
                      'action': actions,
                      }

    out = model.forward(dyna_net_input)
    print(out.shape)
    ...
    n_roll = 2
    states = torch.zeros([B, n_his, obs_dim]).cuda()
    actions = torch.zeros([B, n_his + n_roll, action_dim]).cuda()
    ...
    out = rollout_model(state_init=states,
                        action_seq=actions,
                        dynamics_net=model)
    print(out['state_pred'].shape)

    loss = torch.mean(out['state_pred'])
    loss.backward()

    B = 1
    n_roll = 2
    states = torch.zeros([n_his, obs_dim]).cuda()
    actions = torch.zeros([B, n_his + n_roll, action_dim]).cuda()
    ...
    out = rollout_action_sequences(state_init=states,
                            action_seq=actions,
                            model_dy=model)
    print(out['state_pred'].shape)
