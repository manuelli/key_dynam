...
def keypoints_3D_from_dynamics_model_output(state_pred,  # [B, state_dim] or [state_dim]
                                            K,  # num keypoints
                                            ):  # [B, K, 3]

    has_batch_dim = (len(state_pred.shape) == 2)

    if not has_batch_dim:
        state_pred = state_pred.unsqueeze(0)

    assert len(state_pred.shape) == 2
    B, _ = state_pred.shape
    keypoints = state_pred[:, :(K * 3)].reshape([B, K, 3])
